...
# 추가된 커스텀 음성인식 함수 선언 
from actions import LIGHTON
from actions import LIGHTOFF
from actions import MOTORUP
from actions import MOTORDOWN
    ...
    def main(self):
        ...
        with Assistant(credentials, device_model_id) as assistant:
            ...
            for event in events:
                ...
                # 해당 음성을 인식하면 함수를 실행시키는 커스텀 명령
                if '스탠드 켜'.lower() in str(usrcmd).lower():
                    assistant.stop_conversation()
                    LIGHTON(str(usrcmd).lower())
                if '스탠드 꺼'.lower() in str(usrcmd).lower():
                    assistant.stop_conversation()
                    LIGHTOFF(str(usrcmd).lower())
                if '모터 올려'.lower() in str(usrcmd).lower():
                    assistant.stop_conversation()
                    MOTORUP(str(usrcmd).lower())
                if '모터 내려'.lower() in str(usrcmd).lower():
                    assistant.stop_conversation()
                    MOTORDOWN(str(usrcmd).lower())
